...
def begin_graphics(width=640, height=480, color=formatColor(0, 0, 0), title=None):
    ...
    # This does nothing why is it here --LN
...
def keys_pressed(d_o_e=_root_window.tk.dooneevent,
                 d_w=_tkinter.DONT_WAIT):
    d_o_e(d_w)
    if _got_release:
        d_o_e(d_w)
...
def remove_from_screen(x,
                       d_o_e=lambda arg: _root_window.tk.dooneevent,
                       d_w=_tkinter.DONT_WAIT):
    _canvas.delete(x)
    d_o_e(d_w)
...
def move_by(object, x, y=None,
            ...
            d_w=_tkinter.DONT_WAIT, lift=False):
    if y is None:
        try: x, y = x
