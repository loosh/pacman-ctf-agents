...
class GameState:
    ...
    def makeObservation(self, index):
        ...
        distances = [noisyDistance(pos, state.getAgentPosition(i))
                     for i in range(n)]
    ...
    def initialize(self, layout, numAgents):
        ...
        self.blueTeam = [i for i, p in enumerate(
            positions) if not self.isRed(p)]
...
class CaptureRules:
    ...
    def newGame(self, layout, agents, display, length, muteAgents, catchExceptions):
        ...
        game = Game(agents, display, self, startingIndex=starter,
                    muteAgents=muteAgents, catchExceptions=catchExceptions)
    ...
    def getProgress(self, game):
        blue = 1.0 - (game.state.getBlueFood().count() /
                      float(self._initBlueFood))
        red = 1.0 - (game.state.getRedFood().count() /
                     float(self._initRedFood))
...
class AgentRules:
    ...
    def applyAction(state, action, agentIndex):
        ...
        if next == nearest:
            ...
            # Change agent type
            agentState.isPacman = [isRed, state.isRed(
                agentState.configuration)].count(True) == 1
    ...
    def dumpFoodFromDeath(state, agentState, agentIndex):
        ...
        if not agentState.isPacman:
            raise Exception(
                'something is seriously wrong, this agent isnt a pacman!')
        ...
        def allGood(state, x, y):
            ...
            # loop through agents
            agentPoses = [state.getAgentPosition(
                i) for i in range(state.getNumAgents())]
            ...
            for index in otherTeam:
                ...
                if manhattanDistance(ghostPosition, agentState.getPosition()) <= COLLISION_TOLERANCE:
                    ...
                    if otherAgentState.scaredTimer <= 0:
                        AgentRules.dumpFoodFromDeath(
                            state, agentState, agentIndex)
            ...
            for index in otherTeam:
                ...
                if manhattanDistance(pacPos, agentState.getPosition()) <= COLLISION_TOLERANCE:
                    ...
                    if agentState.scaredTimer <= 0:
                        AgentRules.dumpFoodFromDeath(
                            state, otherAgentState, agentIndex)
...
def readCommand(argv):
  ...
  """
    ...
    redArgs, blueArgs = parseAgentArgs(
        options.redOpts), parseAgentArgs(options.blueOpts)
    ...
    args['agents'] = sum([list(el) for el in zip(
        redAgents, blueAgents)], [])  # list of agents
    ...
    for i in range(options.numGames):
        if options.layout == 'RANDOM':
            ...
            l = layout.Layout(randomLayout(
                int(options.layout[6:])).split('\n'))
...
def loadAgents(isRed, factory, textgraphics, cmdLineArgs):
    ...
    try:
        ...
        module = SourceFileLoader('player' + str(int(isRed)), factory).load_module()
...
def runGames(layouts, agents, display, length, numGames, record, numTraining, redTeamName, blueTeamName, muteAgents=False, catchExceptions=False):
    ...
    for i in range(numGames):
        ...
        if beQuiet:
            ...
            gameDisplay = display
            rules.quiet = False
        g = rules.newGame(layout, agents, gameDisplay,
                          length, muteAgents, catchExceptions)
        g.run()
        if not beQuiet:
            games.append(g)
        ...
        if record:
            ...
            #f = file(fname, 'w')
            components = {'layout': layout, 'agents': [game.Agent(
            ) for a in agents], 'actions': g.moveHistory, 'length': length, 'redTeamName': redTeamName, 'blueTeamName': blueTeamName}
            # f.close()
            print("recorded")
